...
export const performSurgicalUpdate = (originalHtml: string, snippets: { introHtml: string, keyTakeawaysHtml: string, comparisonTableHtml: string, faqHtml?: string, referencesHtml?: string }): string => {
    const parser = new DOMParser();
    ...
    // --- 2. KEY TAKEAWAYS REPLACEMENT (DE-DUPLICATION UPGRADE) ---
    // Aggressively hunt down ANY existing Key Takeaways headers to prevent double-vision.
    const potentialHeaders = Array.from(body.querySelectorAll('h2, h3, h4, h5, h6, strong, b, p strong'));
    const takeawayHeaders = potentialHeaders.filter(el => {
        const txt = el.textContent?.toLowerCase() || '';
        return txt.includes('key takeaways') || txt.includes('key points') || txt.includes('at a glance') || txt.includes('quick summary');
    });

    // Nuke them all
    takeawayHeaders.forEach(header => {
        // Remove the header
        // Also remove the next sibling if it's a list or a container div
        let sibling = header.nextElementSibling;
        if (sibling && (sibling.tagName === 'UL' || sibling.tagName === 'OL' || sibling.tagName === 'DIV' || sibling.tagName === 'P')) {
            sibling.remove();
        }
        header.remove();
    });

    // Create SOTA container
    ...
    // Insert new Takeaways Container
    // Re-finding insertion point:
    const firstH2Recheck = body.querySelector('h2');
    if (firstH2Recheck && firstH2Recheck.parentNode) {
        firstH2Recheck.parentNode.insertBefore(takeawaysContainer, firstH2Recheck);
    } else {
        // fallback
        const intro = body.querySelector('p'); // After first paragraph?
        if (intro && intro.nextSibling) {
             intro.parentNode?.insertBefore(takeawaysContainer, intro.nextSibling);
        } else {
             body.prepend(takeawaysContainer);
        }
    }
    ...
    // --- 5. REFERENCES INJECTION (SOTA) ---
    // Inject at the very bottom, before footer, removing old ones.
    if (snippets.referencesHtml) {
        const tempRefDiv = doc.createElement('div');
        tempRefDiv.innerHTML = snippets.referencesHtml;
        const newRefNode = tempRefDiv.firstElementChild || tempRefDiv;

        // Clean up existing references to avoid duplicates
        const existingRefHeader = Array.from(body.querySelectorAll('h2, h3')).find(el => {
            const text = el.textContent?.toLowerCase() || '';
            return text.includes('references') || text.includes('sources') || text.includes('further reading');
        });

        if (existingRefHeader && existingRefHeader.parentNode) {
            // Remove old ones surgically
            const parent = existingRefHeader.parentNode;
            let sibling = existingRefHeader.nextSibling;
            const toRemove: ChildNode[] = [existingRefHeader];
            
            // Look for lists or divs following the header
            while (sibling && (sibling.nodeName === 'UL' || sibling.nodeName === 'OL' || sibling.nodeName === 'DIV' || sibling.nodeName === 'P')) {
                // Stop if we hit another H2 or any other block element not in our list
        ...
        // Insert new references at the bottom of body content (before any footer tags if they exist inside body)
        const footer = body.querySelector('footer');
        if (footer && footer.parentNode) {
            footer.parentNode.insertBefore(newRefNode, footer);
        } else {
            body.appendChild(newRefNode);
        }
    }
...
export const postProcessGeneratedHtml = (html: string, plan: GeneratedContent, youtubeVideos: any[] | null, siteInfo: SiteInfo, isRefresh: boolean = false): string => {
    ...
    // Remove old AI generated references if they snuck in
...
export const processInternalLinks = (content: string, availablePages: SitemapPage[]): string => {
    // 1. Handle Explicit Placeholders first (High Confidence)
    ...
    processedContent = processedContent.replace(placeholderRegex, (match, slug, text) => {
    ...
    if (availablePages.length > 0 && processedContent.length > 1000) {
        ...
        // We limit this to top 50 pages to keep performance sane in JS
        const linkTargets = availablePages
            .filter(p => p.slug && p.title && p.id)
            .slice(0, 50) 
            .map(p => ({
                url: p.id,
                keywords: [
                    p.title.toLowerCase(),
                    // Extract core phrase from slug e.g. "best-seo-tools" -> "seo tools"
                    p.slug.replace(/-/g, ' ').replace(/^(best|top|guide|review)\s+/i, '')
                ].filter(k => k.length > 4) // Ignore short stopwords
            }));

        // Simple text replacement avoiding existing tags
        linkTargets.forEach(target => {
            target.keywords.forEach(keyword => {
                // Regex explain: 
                // (?![^<]*>) -> Lookahead to ensure we are not inside a tag
                // \b${keyword}\b -> Whole word match
                // (?!<a) -> Don't match if it's already an anchor tag being formed (simple check)
                
                // Limiting to 1 link per target per page
                const escapedKeyword = escapeRegExp(keyword);
                const regex = new RegExp(`(?![^<]*>)\\b${escapedKeyword}\\b`, 'i');
                
                // Check if we already linked this URL? 
                if (!processedContent.includes(`href="${target.url}"`)) {
                     processedContent = processedContent.replace(regex, (match) => {
                        return `<a href="${target.url}" class="internal-link-semantic">${match}</a>`;
                     });
                }
            });
        });
